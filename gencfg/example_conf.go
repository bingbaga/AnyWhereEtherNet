...
func GetExampleEdgeConf(templatePath string, getDemo bool) (mtypes.EdgeConfig, error) {
	...
	econfig = mtypes.EdgeConfig{
		...
		DefaultTTL:         200,
		L2FIBTimeout:       3600,
		IdentityPrivateKey: "6GyDagZKhbm5WNqMiRHhkf43RlbMJ34IieTlIuvfJ1M=",
		ListenPort:         0,
		Transport: mtypes.TransportConfig{
			Protocol: "udp_xor",
		...
		DynamicRoute: mtypes.DynamicRouteInfo{
			...
			SuperNode: mtypes.SuperInfo{
				...
				PeerKeyV4:            "LJ8KKacUcIoACTGB/9Ed9w0osrJ3WWeelzpL2u4oUic=",
				EndpointV6:           "[::1]:3000",
				PeerKeyV6:            "HCfL6YJtpJEGHTlJ2LgVXIWKB/K95P57LHTJ42ZG8VI=",
...
func GetExampleSuperConf(templatePath string, getDemo bool) (mtypes.SuperConfig, error) {
	...
	sconfig = mtypes.SuperConfig{
		NodeName:             "NodeSuper",
		PostScript:           "",
		IdentityPrivateKeyV4: "mL5IW0GuqbjgDeOJuPHBU2iJzBPNKhaNEXbIGwwYWWk=",
		IdentityPrivateKeyV6: "+EdOKIoBp/EvIusHDsvXhV1RJYbyN3Qr8nxlz35wl3I=",
		ListenPort:           3000,
		Transport: mtypes.TransportConfig{
			Protocol: "udp_xor",
			XOR: mtypes.TransportXORConfig{

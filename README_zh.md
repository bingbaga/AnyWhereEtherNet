...
一個底層傳輸可插拔的 Full Mesh Layer2 VPN。  
...
Mode        | Description
------------|:-----
Static Mode | 沒有自動選路，沒有握手伺服器<br>所有 peer 的底層傳輸地址都提前配置好<br>[詳細介紹](example_config/static_mode/README_zh.md)
Super Mode | 此模式是受到[n2n](https://github.com/ntop/n2n)的啟發，分為SuperNode和EdgeNode兩種節點<br>EdgeNode首先和SuperNode建立連線，藉由SuperNode交換其他EdgeNode的資訊<br>由SuperNode執行[Floyd-Warshall演算法](https://zh.wikipedia.org/zh-tw/Floyd-Warshall算法)，並把計算結果分發給EdgeNode<br>SuperNode預設只負責控制面，不是所有資料流量都必須經過它；EdgeNode仍會盡量彼此直連，並依計算出的下一跳轉發。<br>[詳細介紹](example_config/super_mode/README_zh.md)
...
[最新版配置文件说明与使用方式](CONFIG_USAGE_zh.md)

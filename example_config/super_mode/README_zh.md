...
SuperNode 在這個模式下預設是控制面，不是所有資料流量都必須經過它。

- EdgeNode 先向 SuperNode 註冊，並透過它取得 peer 資訊與轉發表
- 取得 peer list 之後，EdgeNode 仍然會盡量彼此直連
- `Normal Packet` 的實際轉發路徑由 `NextHopTable` 決定
- 如果兩個 edge 可以直連，資料會直接 edge-to-edge
- 如果不能直連，或經其他 edge 的成本更低，才會走其他 edge 作為下一跳
- SuperNode 並不是每個封包的固定中繼站
...
也就是說，SuperNode 主要提供拓撲資訊與轉發表；真正的資料路徑仍然由 edge 端根據圖與下一跳自行決定。
...
IdentityPrivateKeyV4           | IPv4通訊使用的私鑰
IdentityPrivateKeyV6           | IPv6通訊使用的私鑰
ListenPort          | udp監聽埠
...
SharedKey                | 和SuperNode通訊用的PreShared Key
EndpointV4           | SuperNode的IPv4 Endpoint
PeerKeyV4             | SuperNode的IPv4公鑰
EndpointV6           | SuperNode的IPv6 Endpoint
PeerKeyV6             | SuperNode的IPv6公鑰
